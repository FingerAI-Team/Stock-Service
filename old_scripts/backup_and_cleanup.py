...
def cleanup_database_with_backup():
    ...
    if null_hash_records:
        ...
        for record in tqdm(null_hash_records, desc="해시값 업데이트"):
            ...
            # 중복 체크
            postgres.db_connection.cur.execute(
                f"SELECT COUNT(*) FROM {table_name} WHERE hash_value = %s",
                (hash_value,)
            )
            existing_count = postgres.db_connection.cur.fetchone()[0]
            
            if existing_count > 0:
                # 중복된 해시값인 경우, conv_id를 포함한 고유한 해시값 생성
                unique_hash_value = hashlib.md5(
                    f"{user_id}_{content}_{date}_{conv_id}".encode()
                ).hexdigest()
                hash_value = unique_hash_value
                duplicate_count += 1
        ...
        if duplicate_count > 0:
            logger.info(f"⚠️ {duplicate_count}개 레코드는 중복으로 인해 고유 해시값으로 변경됨")
